...
@Component({
  selector: 'app-dam-pharm-customers-edit',
...
export class EditComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    this._updateSub = this._routes.paramMap
      .switchMap((params: ParamMap) => {
        return this._customersService.update(this.customer);
      })
      .subscribe(
      data => {
        if (isObject(data)) {
          ...
          this._utils.notyf('success','Customer Update Sucessfuly');
        }
      }
      );
  }
  ...
  onDelete() {
    ...
    this._deleteSub = this._customersService.delete(this.customer.id)
      ...
      this._utils.notyf('success','Customer Deleted Sucessfuly');
  }
